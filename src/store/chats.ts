...
interface ChatsStore {
  ...
  createSession: (participantIds: string[]) => { success: boolean; chatId?: string; outcome: string };

  /** Add an agent to an existing session. */
  joinSession: (chatId: string, agentId: string) => { success: boolean; outcome: string };

  /** Remove an agent from a session. Cleans up if <2 remain. */
  leaveSession: (chatId: string, agentId: string) => { success: boolean; outcome: string };

  /** Send a message in a chat session. */
  sendMessage: (chatId: string, message: ChatMessage) => { success: boolean; outcome: string };
  ...
  getNearbyAgents: (agentId: string, maxDistance?: number) => Array<{ id: string; name: string; distance: number; inChat?: string }>;
}
...
export const useChatsStore = create<ChatsStore>((set, get) => ({
  ...
  createSession: (participantIds) => {
    if (participantIds.length < 2) {
      return { success: false, outcome: 'At least 2 participants are required.' };
    }
  ...
  joinSession: (chatId, agentId) => {
    ...
    if (!session) return { success: false, outcome: `Session ${chatId} not found.` };
    ...
    set((state) => ({
      sessions: {
        ...state.sessions,
        [chatId]: { ...session, participants: [...session.participants, agentId] },
      },
    }));

    agentsStore.updateChatId(agentId, chatId);
    return { success: true, outcome: `Joined chat. Participants: ${session.participants.length + 1}` };
  },

  leaveSession: (chatId, agentId) => {
    const session = get().sessions[chatId];
    if (!session) return { success: false, outcome: `Session ${chatId} not found.` };
    if (!session.participants.includes(agentId)) return { success: false, outcome: 'Not in this chat.' };
  ...
  sendMessage: (chatId, message) => {
    ...
    if (!session.participants.includes(message.id)) {
      return { success: false, outcome: 'Not in this chat. Use start_chat first.' };
    }
  ...
  getAgentSession: (agentId) => {
    return Object.values(get().sessions).find((s) => s.participants.includes(agentId));
  },
  ...
  getNearbyAgents: (agentId, maxDistance = 100) => {
    ...
    return agents
      ...
      .map((other) => {
        const distance = Math.sqrt(Math.pow(other.x - agent.x, 2) + Math.pow(other.y - agent.y, 2));
        const chat = Object.values(sessions).find((s) => s.participants.includes(other.id));
